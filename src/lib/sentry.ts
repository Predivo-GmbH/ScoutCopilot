/**
 * Sentry error/crash monitoring (PILOT, added 2026-08-13, STAGING ONLY).
 *
 ...
 * Actions secret). It is NOT prod-host-gated: we WANT staging errors during the
 * pilot, and tag each event with `environment` = hostname so staging vs production
 * stay distinguishable in one project.
 *
...
export function initSentry(): void {
  ...
  Sentry.init({
    ...
    release: RELEASE,
    environment:
      window.location.hostname === 'scoutcopilot.com' ? 'production' : window.location.hostname,
