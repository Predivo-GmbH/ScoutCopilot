#!/usr/bin/env node
/**
 * auth-captcha.mjs — read and flip ScoutCopilot's Supabase Auth CAPTCHA switch, without ever
 * rendering a secret.
 *
 ...
 * how ScoutCopilot's gets flipped, read back and rolled back, on the record, by CI, with no secret
 * rendered anywhere.
 *
 * WHAT IT DOES
...
const PROJECTS = {
  production: { ref: 'rlcsuqwqzoqjykdiqjye', name: 'ScoutCopilot production' },
  staging: { ref: 'ysdaeexwhbwlbatcscqn', name: 'ScoutCopilot staging' },
}

/** PUBLIC Cloudflare Turnstile site key for ScoutCopilot — safe to record, it ships in every bundle. */
const SITE_KEY = '0x4AAAAAAEz4tOM8hsZWk2Ed'
...
async function mgmt(path, init = {}) {
  if (!MGMT_CANDIDATES.length) {
    throw new Error('No Supabase management token in the environment (SUPABASE_ACCESS_TOKEN).')
  }
...
function turnstileSecret() {
  ...
  if (!value) {
    throw new Error(
      'TURNSTILE_SECRET is not set. It is the Cloudflare Turnstile SECRET key for the ScoutCopilot ' +
  ...
  if (codes.includes('invalid-input-secret') || codes.includes('missing-input-secret')) {
    throw new Error(
      `Cloudflare REFUSED this secret (${codes.join(', ')}). TURNSTILE_SECRET is not the secret key ` +
        `for the ScoutCopilot widget (site key ${SITE_KEY}). Enabling captcha with it would reject ` +
