...
12. **`generate-photo` recorded every unauthenticated request as an application error** — measured 2026-09-02: 4,931 rows of `"Missing or invalid Authorization header"` against 56–57 for each of the other seven logging functions, i.e. **94% of every error this product had ever logged**. The photo path itself was never broken; the callers were our own monitoring (a Cockpit health prober from the edge runtime, the Playwright edge-function sweep, curl probes), which arrive with no `Authorization` header because supabase-js 2.112.3 builds the functions fetch with `omitApiKeyAsBearer: true`, so a `sb_publishable_` key is never sent as a Bearer token and a session-less call arrives bare. Because `verify_jwt = false` project-wide (ES256 JWTs the edge runtime cannot verify), the endpoint is publicly reachable, so anyone who found the URL could grow the table at will. **Two things were wrong, and the second one mattered more:** every row also carried an empty `context`, and the UX Scout (`production-monitor/scripts/ux-scout.mjs`) classifies a pattern as anonymous-and-dismissible when `context->>'user_id'` is null on all of it — so a *genuine* ScoutCopilot fault would have been filed as a bot probe too. Resolved 2026-09-02 in `generate-photo/index.ts` (commit `4ec9bfe`, function v39): an `AuthError` returns its 401 unchanged but no longer writes an `error_log` row (Supabase's own `function_edge_logs` already records these with status, IP and user agent), and a genuine error now carries `user_id` / `organization_id` / `has_auth`. Verified live: the same anonymous POST wrote a row before the deploy and writes none after. **Still open** — the other seven functions share the pattern at ~56 rows each; the clean fix is in `_shared/auth.ts` + `_shared/error-log.ts`.
